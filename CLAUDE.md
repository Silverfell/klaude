...
- `upgrade.sh` is the repair tool for a damaged `changes.db`: it re-creates any trigger or view the shipped schema defines and the database lacks, idempotently, and fails on a missing table. `/close` only detects and points at it; the protocols never run `CREATE` statements themselves.
...
- The command files (`template/klawde.md`, `template/close.md`) and the contract are rewritten for Codex by `rewrite_codex` in both scripts, which rewrites only backticked `/klawde`, `/close`, and `.claude/commands/*.md` references and the bare schema path. Never name `CLAUDE.md` in a command file (Codex has `AGENTS.md`), and always backtick `/klawde`, `/close`, and `.claude/` paths, including inside code comments. The suite's Codex drift test greps the generated files for leftovers.
